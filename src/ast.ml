...
let b2s {id; typ} =
  Code.fmt "%s : %s" (Symbol.format id) (Typ.format typ)
...
and term = Id of Symbol.t
         ...
         | App of {rator : Symbol.t; rands : term list}    (* term is an identifier *)
...
let toStringBindOcc {id; typ} =
  ...
  in
  Code.fmt "%s : %s" ids typs

(* make a string for an assignment: M = N
*
let toStringEqual s1 s2 = Code.fmt "%s = %s" s1 s2
*)
...
let rec toString term =
  ...
  | App {rator; rands} ->
    ...
    in
    Code.fmt "%s(%s)" ras commaSepRands
  ...
  | If {expr; thn; els} ->
    ...
    let p2 = toString thn in
	  let p3 = toString els
    in
    Code.fmt "if0 %s then %s else %s" p1 p2 p3

  | Or {left; right} ->
    let p1 = toString left in
    let p2 = toString right
    in
    Code.fmt "(%s or %s)" p1 p2

  | And {left; right} ->
    let p1 = toString left in
    let p2 = toString right
    in
    Code.fmt "(%s and %s)" p1 p2

  | Let {decl; body} ->
    let ds = toStringDec decl in
    let bs = toString body
    in
    Code.fmt "let %s in %s" ds bs
...
and ppBlock out indent body =
  ...
  | Block {decls; statements} ->
    ...
    let pdecs = fun {id; typ} ->
      Util.writeln(out, nd, Code.fmt "%s %s;" (Typ.format typ) (Symbol.format id))
    in
...
and ppStatement out indent statement =
  ...
  | Assign {id; expr} ->
    ...
    if String.length exprs > 20 then
      (
        Util.write(out, indent, Code.fmt "%s = " ids);
        ppTerm out indent expr;
        Util.writeln(out, indent, ";")
      )
    else
      Util.writeln(out, indent, Code.fmt "%s = %s;" ids exprs)

  | While {expr; statement} ->
    let ts = toString expr
    in
    (
      Util.writeln(out, indent, Code.fmt "while (%s)" ts);
  ...
  | IfS {expr; thn; els} ->
    ...
    (
      if String.length exprs > 20 then
        ...
        Util.writeln(out, indent, Code.fmt "if (%s) then" exprs)
    );
  ...
  | Call {rator; rands} ->
    ...
    let calls = Code.fmt "call %s(%s);" rators (toStringList rands toString)
    in
  ...
  | Print expr ->
    let exprs = toString expr
    in
    Util.writeln(out, indent, Code.fmt "print %s;" exprs)
...
and ppTerm out indent term =
  ...
  | App {rator; rands} ->
    ...
    in
    Util.writeln(out, indent, Code.fmt "%s(%s)" rators randss)

(* If expression.
*)
  | If {expr; thn; els} ->
    let topLine = Code.fmt "if %s then" (toString expr)
    in
    (
...
*)
  | And {left; right} ->
    ...
    in
    Util.writeln(out,indent, Code.fmt "(%s and %s)" ls rs)
  ...
  *)
  | Or {left; right} ->
    ...
    let rs = toString right
    in
    Util.writeln(out,indent, Code.fmt "(%s or %s)" ls rs)
      ...
      | ValBind {bv; defn} ->
        ...
        (match defn with
         | Id _ | Literal _ | If _ | And _ | Or _ | App _ ->
           ...
           in
           Util.writeln(out, indent, Code.fmt "%s = %s" lhs defns)

         | Let _ ->
           (
             Util.writeln(out, indent, Code.fmt "%s =" lhs);
