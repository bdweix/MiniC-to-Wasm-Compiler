...
let compilerOptions () =
  ...
  | 3 -> (
    ...
    | "-nocheck" ->
        ...
        2
    | "-t" ->
        Debug.debugLexer := true ;
        2
    | anythingelse ->
        failwith (Code.fmt "Unknow compiler option %s.\n\n" Sys.argv.(1)) )
...
let parseFile fileName =
  ...
  let ast =
    if !Debug.debugLexer then
      let _ = Debug.dumpTokens Lexer.token lexbuf in
      Ast.Program []
...
let compile () =
  ...
  let _ =
    if !typeChecking then
      let msg =
        try
          let _ = Static.typeCheck typeEnv ast in
          "\nThe program is well-typed.\n"
        with Static.TypeError s ->
          let _ = print_string s in
          let _ = Util.writeln (dbgOut, 0, s) in
          failwith "Compilation failed.\n"
      in
      if !Debug.debug then Util.writeln (dbgOut, 0, msg) else ()
    else (* No type checking, pretend the program was well-typed. *)
      ()
  in
  ...
  let _ =
    match
      List.exists
        (fun (Ast.Procedure Ast.({id; formals})) ->
          Symbol.format id = "main" && formals = [] )
        ps
    with
    | true -> ()
  ...
  *)
  ...
  (* Convert to Wasm here *)
  let wasm = Wasm.translate quads in
  let _ = Util.writeln (dbgOut, 0, "\nAfter the WASM phase:\n") in
  let _ = Wasm.dumpInstructionStream wasm in
  ...
  *)
  let _ = Mips.Codestream.emit (filename, codestream) in
  ()
