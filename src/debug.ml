...
let debugSetup fname =
  if !debug then
    ...
    let message = Code.fmt "Compiling %s in debug mode." fname in
    let _ = Code.pfmt "\n%s See file %s.\n" message dbgName in
    let date = Util.dateAndTime() in
    let prefix = Code.fmt "%s: %s. " Util.compilerName date in
    let _ = output_string dbgOut (Code.fmt "%s%s\n" prefix message)
    in
    dbgOut
  else
    stdout
...
let dumpEnv tenv =
  let printer id typ =
    ...
    in
    output_string !dbgout (Code.fmt "%s : %s\n" ids typs)
  in
