...
module Operation = struct
  ...
  type t =
    ...
    *)
     ...
     *)
    | Lb of {rd: opnd; rs: opnd}
    (* NB: rs will be an indirect opnd *)
  ...
  (* NB: Operand must be a string. *)
  
  (* format code.  The first three functions are local helpers.
 *)
  let fmt1 fmtstring opnd = fmt fmtstring (Opnd.format opnd)
...
module Instruction = struct
  ...
  let toInstruction (lab, opn, comment) : t =
    {label= lab; operation= opn; comment}
  ...
  let format instr =
    let opnfmt = Opn.format instr.operation in
    match instr with
