...
let formatCommaSeparated formatter items =
  let rec repeat items =
    ...
    | [] -> ""
    | [item] -> formatter item
    | item :: items -> Code.fmt "%s, %s" (formatter item) (repeat items)
  in
  repeat items
