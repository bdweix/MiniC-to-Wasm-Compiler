...
let testCodegen n =
  let codestream = Codegen.translate TestCodegen.pgms.(n) in
  let filename = "see.mc"
  in
...
let dispatch () =
  ...
  | "lift" -> testLift n
  | "control" -> testControl n
  | "codegen" -> testCodegen n
